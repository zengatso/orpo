...
class ORPO(object):
            ...
            chosen = [
                self.tokenizer.apply_chat_template(
                    [
                        ...
                        {"role": "assistant", "content": item_chosen},
                    ],
                    tokenize=False,
                )
                for item_prompt, item_chosen in zip(
                    examples[prompt_key], examples["chosen"]
                )
            ]
            rejected = [
                self.tokenizer.apply_chat_template(
                    [
                        {"role": "user", "content": item_prompt},
                        {"role": "assistant", "content": item_rejected},
                    ],
                    tokenize=False,
                )
                for item_prompt, item_rejected in zip(
                    examples[prompt_key], examples["rejected"]
                )
            ]
        else:
            prompt = [
                self.tokenizer.apply_chat_template(
                    [item[0]], tokenize=False, add_generation_prompt=True
                )
                for item in examples["chosen"]
            ]
            chosen = [
                self.tokenizer.apply_chat_template(item, tokenize=False)
                for item in examples["chosen"]
            ]
            rejected = [
                self.tokenizer.apply_chat_template(item, tokenize=False)
    ...
    def filter_dataset(self, examples: Union[List, Dict]):
        if "instruction" in examples.keys():
            ...
            prompt_length = self.tokenizer.apply_chat_template(
    ...
    def prepare_trainer(self):
        ...
        arguments = TrainingArguments(
            ...
            if self.is_test
            else "no",  # batch size for evaluation
